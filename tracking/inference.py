...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        num0fParticles = 0
        self.particles = []
        while num0fParticles < self.numParticles:
            for legalboardPos in self.legalPositions:
                if num0fParticles < self.numParticles:
                    #Initializes particles to positions
                    self.particles.append(legalboardPos)
                    num0fParticles += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            #all particles should be updated so that the ghost appears in its prison cell
            ...
            if not any(allPossible.values()):
                ...
                while num0fParticles < self.numParticles:
                    ...
                    sample.append(util.sample(allPossible))
                    num0fParticles += 1
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            changedParticles.append(util.sample(newPosDist))
        self.particles = changedParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        legalGhostPositions = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        #ensures even placement of particles across the board
        random.shuffle(legalGhostPositions)
        num0fParticles = 0
        self.particles = []
        while num0fParticles < self.numParticles:
            for legalboardPos in legalGhostPositions:
                if num0fParticles < self.numParticles:
                    #Initializes particles to positions
                    self.particles.append(legalboardPos)
                    num0fParticles += 1
